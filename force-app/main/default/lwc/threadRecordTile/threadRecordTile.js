...
export default class ThreadRecordTile extends NavigationMixin(LightningElement) {
    @api recordId;
    @api mainArea;
    // @api ticketUsers = [];
    ...
    @api timelineContainerClass = 'threadTimelineClass flexReverse slds-m-top_medium';
    @api ticketMsg;
    @api threadView;
    userId = USER_ID;
    closedThread;
    ...
    @api
    callRefreshFile(){
        const el = this.template.querySelector('c-thread-record-tile-message');
        el.callRefreshFile();
    }
    handleResolve(event) {
